...
\noindent\makebox[\textwidth]{%
    ...
    \begin{tikztimingtable}[timing/slope=.3]
      %       |  D0   |  D1   |  D0   |  D1   |    MES 01     |
      %       |   ACK   10   |     RST 01   | 0 Reset P11 | Idle
      Din  & L 8{1.5L} 8{1.5H} 8{1.5L} 8{1.5H} 4{1.5L} 4{1.5H}
               4{1.5H}4{1.5L} 8{1.5L}8{1.5H} 8{1.5L}4{1.5L}6{1.5H} 8{1.5H} H \\
      CLK  & C 8{1.5C} 8{1.5C} 8{1.5C} 8{1.5C} 4{1.5C} 4{1.5C}
               4{1.5C}4{1.5C} 3L3{3C}  4{3C}   4{3C}  2{3C}  3{3C}   4{3C} C \\
      Bus  & D{}{12D{Data 0}}{12D{Data 1}}{12D{Data 0}}{12D{Data 1}}{12D{MES 01}}
               {12D{ACK 10}}{27D{Reset 010}} {36D{Reset 11}}D \\
      \extracode
        \begin{pgfonlayer}{background}
          \begin{scope}[semitransparent,semithick,dashed,color=red]
            \vertlines{1,7,...,71}
        ...
        \begin{scope}
          ...
          \node [rotate=45] at (\base+36,0) {Reset$_{??}$};
          \node [rotate=45] at (\base+42,0) {Reset$_{1}$};
          \node [rotate=45] at (\base+48,0) {Reset$_{2}$};
          \node [rotate=45] at (\base+56,0) {Reset$_{3}$};
          \node [rotate=45,color=black] at (\base+62,0) {Idle (L)};
        \end{scope}
    \end{tikztimingtable}
}
...
node is seeking either a {\tt 011} or a {\tt 0111} sequence. This will reliably
...
\begin{figure}[htb]
  ...
  \begin{tikztimingtable}[timing/slope=.3,timing/wscale=1.0]
    ...
    \extracode
      ...
      \end{scope}
  \end{tikztimingtable}
  \caption[Lof Entry]{{\em TX 0$\rightarrow$1 False MES~~}
    We assume the RX latches 0 at time 4, creating the problem. The TX may
    possibly be reset at time 7 if it latched 1, here we assume not. At time 8
    the control layer detects a change between {\sc latch2} and {\sc drive1},
    alerting it to the issue and entering Reset.

    \hspace{1em}
    We show the next TX half-bit to be~{\tt 0} as if it was a~{\tt 1}, the TX
    layer would reset at time 10. If the next half-bit had been~{\tt 1}, it
    would also introduce the possibility of the RX node latching a~{\tt 1} at
    time 8 and failing its ACK attempt.  We show the next TX half-bit as
    a~{\tt 1} (that is, the TX layer was sending the Message~End~Sequence); if
    it had been~{\tt 0} the TX layer would have reset at time 14 when its {\sc
    latch2} latched a~{\tt 1} instead of a~{\tt 0}.
...
discussion in \ref{sec:reset-hung}~\nameref{sec:reset-hung} for more details.
...
seen by the controller\footnote{
  Note that the arbitration edges are defined as {\sc latch1}, {\sc drive1},
  and {\sc latch2}, and thus will not count towards this limit.
