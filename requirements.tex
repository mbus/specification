...
\begin{subfigure}{\textwidth}
    ...
    \begin{tikztimingtable}[timing/slope=.3,timing/wscale=1.0]
      ...
      ~~~~ & {5D{Data 0}}{32D{End Sequence}}{60D{Acknowledgement Attempt}}{28D{Error (Forwarding)}}D \\
      %\\
      ...
      \extracode
        ...
        \begin{scope}
          ...
          \node [rotate=45] at (\pgfmathresult, 0)  {R0-L};
          \pgfmathparse{\pgfmathresult+8}
          \node [rotate=45] at (\pgfmathresult, 0)  {RI-1};
          \pgfmathparse{\pgfmathresult+8}
          \node [rotate=45] at (\pgfmathresult, 0)  {RI-1};
          \pgfmathparse{\pgfmathresult+8}
          %\node [rotate=45] at (\base, 0) {Reset-S0-D};
...
transmitter were to send a 0 as the next data bit, it would detect the error
at {\sc latch1} at time 16, however here we examine it sending another 1.
    }
