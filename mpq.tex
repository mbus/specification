...
\subsection{\proto Configuration Registers (Reg \#240--255)}
\label{cmd:conf}
...
of each register address are \hlc[lightgreen]{\texttt{1111}} followed by four
bits that identify the \hlc[lightergreen]{command type}. All
\hlc[lightgray]{reserved} bits should be treated as RZWI.
...
\begin{bytefield}{8}
  \bitheader{0-7} \\
  \colorbitbox{lightgreen}{4}{1111}
  \colorbitbox{lightergreen}{4}{0000}
\end{bytefield}
~
\begin{bytefield}{24}
  \bitheader{0-23} \\
  \colorbitbox{lightgray}{24}{Reserved}
\end{bytefield}
\hfill\textbf{Default: \texttt{0x000000}}
\\

\subsubsection{Reg \#241: Bulk Memory Message Control}
\label{cmd:conf-mem-ctrl}

\begin{bytefield}{8}
  \bitheader{0-7} \\
  \colorbitbox{lightgreen}{4}{1111}
  \colorbitbox{lightergreen}{4}{0001}
\end{bytefield}
~
\begin{bytefield}{24}
  \bitheader{0-23} \\
...
\subsubsection{Reg \#242--245: Reserved}
Writes to reserved registers should be ignored. Reads from reserved registers
should return all {\tt 0}.

\subsubsection{Reg \#246--247: Memory Alert}
\label{cmd:conf-memory-alert}

\begin{bytefield}{8}
  \bitheader{0-7} \\
  \colorbitbox{lightgreen}{4}{1111}
  \colorbitbox{lightergreen}{3}{011}
  \colorbitbox{lightergreen}{1}{0}
\end{bytefield}
~
\begin{bytefield}{24}
  \bitheader{0-23} \\
  \bitbox{2}{LU}
  \bitbox{6}{Length-1}
  \bitbox{14}{Buffer Start Address [15:2]}
  \colorbitbox{lightgray}{1}{\begin{sideways}{\tiny RSV}\end{sideways}}
  \colorbitbox{lightgray}{1}{\begin{sideways}{\tiny RSV}\end{sideways}}
\end{bytefield}
\hfill\textbf{Default: \texttt{0x000000}}
...
\begin{bytefield}{8}
  ...
  \colorbitbox{lightgreen}{4}{1111}
  \colorbitbox{lightergreen}{3}{011}
  \colorbitbox{lightergreen}{1}{1}
\end{bytefield}
~
\begin{bytefield}{24}
  \bitheader{0-23} \\
  \bitbox{1}{\begin{sideways}{\tiny VLD}\end{sideways}}
  \bitbox{1}{\begin{sideways}{\tiny ISEN}\end{sideways}}
  \colorbitbox{lightcyan}{2}{CH}
  \bitbox{4}{\tiny Generating Prefix}
  \bitbox{16}{Buffer Start Address [31:16]}
\end{bytefield}
\hfill\textbf{Default: \texttt{0x000000}}
\\

\subsubsection{Reg \#248--255: Memory Stream Configuration}
...
\begin{bytefield}{24}
  ...
  \bitbox{2}{LU}
  \bitbox{6}{Length-1}
  \bitbox{14}{Write Address Counter [15:2]}
  \colorbitbox{lightgray}{1}{\begin{sideways}{\tiny RSV}\end{sideways}}
  \colorbitbox{lightgray}{1}{\begin{sideways}{\tiny RSV}\end{sideways}}
\end{bytefield}
\hfill\textbf{Default: \texttt{0x000000}}
...
\begin{bytefield}{24}
  ...
  \colorbitbox{lightgray}{1}{\begin{sideways}{\tiny RSV}\end{sideways}}
  \bitbox{4}{\tiny Alert Dest Prefix}
  \bitbox{16}{Write Address Counter [31:16]}
\end{bytefield}
...
\begin{itemize}
  ...
  \item {\tt \{R1XX1[21]\}: Double Buffer (DBLB).}
    ...
    {\tt WRP}.
  \item {\tt \{R1XX1[19:16]\}: Alert Dest Prefix.}
    \subitem Defines which node to send alerts to. If the alert prefix is set
    to broadcast {\tt 0000} or full-prefix {\tt 1111}, the alert is
    suppressed. Alerts are sent whenever the end of the buffer is reached.  If
...
\subsubsection{Memory Read, Memory Alert}
\label{cmd:mem-read}
\label{cmd:mem-alert}

\emph{If Bit~23 is LOW, command is a Memory Read:}
...
%  \bitheader{0,31} \\
  ...
  \bitbox{1}{\begin{sideways}{\tiny ALRT}\end{sideways}}
  \colorbitbox{lightgray}{3}{\tiny RS\\VD}
  \bitbox{20}{Length-1}
...
prepended to the reply (generating a \nameref{cmd:mem-bulk-write} command). If
the third word is omitted, data is immediately placed on the bus (generating a
\nameref{cmd:mem-stream-write} command).
...
\bigskip
\noindent
\emph{If Bit~23 is HIGH, command is a Memory Alert:}
...
\end{bytefield}
~
\begin{bytefield}[bitwidth=.4em]{64}
%  \bitheader{0,31} \\
  \bitbox[]{2}{\tiny ~ \\ ~ \\ 31}
  \bitbox[]{1}{\tiny ~ \\ ~ \\ \tt \bf \dots}
  \bitbox[]{3}{}
  ...
  \bitbox[]{1}{\tiny ~ \\ ~ \\ 0}
  \\
  \bitbox{8}{\tiny \bus Source Address}
  \bitbox{1}{\begin{sideways}{\tiny ALRT}\end{sideways}}
  \colorbitbox{lightgray}{3}{\tiny RS\\VD}
  \bitbox{20}{Length-1}
  \bitbox{30}{Buffer Start Address}
  \colorbitbox{lightgray}{1}{\footnotesize \tt Z}
  \colorbitbox{lightgray}{1}{\footnotesize \tt Z}
  \\
...
The first byte specifies the node and channel that generated the alert.

For streaming alerts, those with a source address of the form {\tt
8'bXXXX01XX}, the second word specifies the beginning of the buffer that
generated the alert on the source node. Bits~19:0 of the first word indicate
the length of the buffer with valid data. Length will either be the entire
buffer size, or half of the buffer if {\tt DBLB} is active for this stream.

If bits~27:26 of the first word are not {\tt 01}, the message is undefined and
should be ignored and NAK'd.
