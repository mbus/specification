...
\subsection{\proto Configuration Registers (Reg \#192--255)}
\label{cmd:conf}
...
\subsubsection{Reg \#239: Alert}
\label{cmd:conf-alert}

\begin{bytefield}{8}
  \bitheader{0-7} \\
  \colorbitbox{lightgreen}{2}{11}
  \colorbitbox{lightergreen}{6}{101111}
\end{bytefield}
~
\begin{bytefield}{24}
  \bitheader{0-23} \\
  \bitbox{8}{\scriptsize ``Alert From'' Address}
  \colorbitbox{lightgray}{16}{Reserved}
\end{bytefield}
\hfill\textbf{Default: \texttt{0x000000}}
\\
...
\subsubsection{Reg \#240--255: Memory Stream Configuration}
...
\begin{itemize}
  \item {\tt \{RXX01[15:0], RXX00[15:2], 2'b00\}: Write Buffer.}
    \subitem Pointer to the beginning of a buffer in memory.
  \item {\tt \{RXX00[23:16]\}: Alert Address.}
    ...
    buffer is reached.
  \item {\tt \{RXX10[19:0]\}: Buffer Length-1.}
    \subitem Defines the length of the buffer.
  \item {\tt \{RXX10[23]\}: Enable (EN).}
  ...
  \item {\tt \{RXX10[21]\}: Double Buffer (DBLB).}
