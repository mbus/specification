...
All \bus master nodes must permit a minimum of 1~K (1024~bits) of data to be
...
received after Begin~Transmission at the master node's {\tt CLK\_IN} port up
to and including the last bit latched (Request~Interrupt in
\cref{fig:interrupt}). The counter shall be an {\em inclusive} counter,
that is the master node shall not Interrupt until it receives the $N+1$th
bit.
...
\cref{fig:interrupt}). The receiving state machine should not transition
...
During normal arbitration, when the arbitration is resolved the master node's
{\tt DIN} line should be low no matter who is participating. If the master
node finds its {\tt DIN} line is high on the arbitration edge, it would
indicate that no one is participating. The master node must treat this as an
error and reset the bus.

The master node {\bf must not} immediately interrupt the bus, however. As
...
potential issues related to half-waking power gated nodes, the master node
must wait until Begin~Transmission before interrupting the bus.

The control bits {\bf must} be driven by the master node. They {\bf must} be
{\tt 00}, general error.
